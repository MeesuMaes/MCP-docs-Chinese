...
提示使服务器能够定义可重用的提示模板和工作流程，客户端可以轻松地将这些模板和工作流程呈现给用户和 LLM。它们提供了一种标准化和共享常见 LLM 交互的强大方式。

<Note>
  提示被设计为**用户控制**，意味着它们从服务器暴露给客户端，目的是让用户能够明确选择使用它们。
</Note>
