---
title: "采样"
description: "让您的服务器从 LLM 请求完成"
---

采样是 MCP 的一项强大功能，允许服务器通过客户端请求 LLM 完成，从而在保持安全性和隐私性的同时实现复杂的代理行为。

<Info>
  MCP 的此功能目前尚未在 Claude Desktop 客户端中得到支持。
</Info>
...
- `hints`：模型名称建议数组，客户端可以使用这些建议选择合适的模型：
  ...
  - 客户端可以将提示映射到来自不同提供者的等效模型
  - 多个提示按偏好顺序评估

- 优先级值（0-1 标准化）：
  - `costPriority`：降低成本的重要性
  - `speedPriority`：低延迟响应的优先级
  - `intelligencePriority`：高级模型能力的优先级

客户端根据这些偏好及其可用模型最终选择模型。

### 系统提示

可选的 `systemPrompt` 字段允许服务器请求特定的系统提示。客户端可以修改或忽略此字段。
...
- `maxTokens`：最大生成令牌数
- `stopSequences`：停止生成的一组序列
- `metadata`：特定于提供者的额外参数
