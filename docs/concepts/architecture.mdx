...
模型上下文协议（MCP）建立在灵活、可扩展的架构之上，使 LLM 应用程序与集成之间的无缝通信成为可能。本文档涵盖了核心架构组件和概念。
...
2. **HTTP 与 SSE 传输**
   - 使用服务器推送事件（Server-Sent Events）进行服务器到客户端的消息传递
