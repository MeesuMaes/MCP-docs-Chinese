...
传输在模型上下文协议（MCP）中为客户端与服务器之间的通信提供了基础。传输层负责处理消息发送和接收的底层机制。
...
SSE 传输通过 HTTP POST 请求实现客户端到服务器的通信，并支持服务器到客户端的流式传输。
...
MCP 使得实现特定需求的自定义传输变得简单。任何传输实现只需符合 Transport 接口即可：
