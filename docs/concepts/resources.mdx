...
资源是模型上下文协议（MCP）中的核心原语，允许服务器暴露客户端可读取并用作 LLM 交互上下文的数据和内容。

<Note>
  资源被设计为**应用程序控制**，这意味着客户端应用程序可以决定如何以及何时使用它们。
  不同的 MCP 客户端可能以不同方式处理资源。例如：
  - Claude Desktop 当前要求用户在资源可以使用之前明确选择它们
  - 其他客户端可能根据启发式方法自动选择资源
  - 某些实现甚至可能允许 AI 模型本身决定使用哪些资源

  服务器作者在实现资源支持时，应准备好处理这些交互模式中的任何一种。为了自动将数据暴露给模型，服务器作者应使用**模型控制**的原语，例如 [工具](./tools)。
</Note>
...
服务器通过 `resources/list` 端点暴露具体资源的列表。每个资源包括：
...
对于动态资源，服务器可以暴露客户端可用来构造有效资源 URI 的 [URI 模板](https://datatracker.ietf.org/doc/html/rfc6570)：

```typescript
{
  uriTemplate: string;   // 遵循 RFC 6570 的 URI 模板
  name: string;          // 该类型资源的易读名称
  description?: string;  // 可选描述
...
要读取资源，客户端使用资源 URI 发起 `resources/read` 请求。
...
<Tip>
  服务器可以对一个 `resources/read` 请求返回多个资源。例如，这可以用来在读取目录时返回目录内的文件列表。
</Tip>
