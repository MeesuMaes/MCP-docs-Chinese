...
在本教程中，我们将构建一个简单的 MCP 天气服务器，并将其连接到主机 Claude for Desktop。我们将从基本设置开始，然后逐步介绍更复杂的使用场景。
...
<Note>
服务器可以连接到任何客户端。我们在这里选择了 Claude for Desktop 以简化操作，但我们还提供了[构建您自己的客户端](/quickstart/client)指南以及[其他客户端列表](/clients)。
</Note>

<Accordion title="为什么选择 Claude for Desktop 而不是 Claude.ai？">
因为服务器是本地运行的，MCP 目前仅支持桌面主机。远程主机的开发正在积极进行中。
</Accordion>
...
`FastMCP` 类使用 Python 类型提示和文档字符串自动生成工具定义，使创建和维护 MCP 工具变得更加简单。

### 辅助函数

接下来，让我们添加用于查询和格式化美国国家气象服务 API 数据的辅助函数：
...
指令: {props.get('instruction', '未提供具体指令')}
"""
```
...
<Note>
Claude for Desktop 目前尚未在 Linux 上可用。Linux 用户可以继续进行[构建客户端](/quickstart/client)教程，以构建一个连接到我们刚刚构建的服务器的 MCP 客户端。
</Note>

首先，确保您已安装 Claude for Desktop。[您可以在此处安装最新版本。](https://claude.ai/download) 如果您已经安装了 Claude for Desktop，**请确保已更新到最新版本。**

我们需要为 Claude for Desktop 配置您想使用的 MCP 服务器。为此，请使用文本编辑器打开位于 `~/Library/Application Support/Claude/claude_desktop_config.json` 的 Claude for Desktop 应用程序配置文件。如果文件不存在，请创建它。
...
然后，您将在 `mcpServers` 键中添加您的服务器。只有至少配置了一个服务器，Claude for Desktop 中才会显示 MCP UI 元素。
...
确保运行 `npm run build` 来构建您的服务器！这是让服务器连接的重要一步。
...
```json Node
{
    "mcpServers": {
        "weather": {
            "command": "node",
            "args": [
                "/绝对/路径/到/父文件夹/weather/build/index.js"
            ]
        }
    }
}
...
```json Node
{
    "mcpServers": {
        "weather": {
            "command": "node",
            "args": [
                "C:\\路径\\到\\父文件夹\\weather\\build\\index.js"
            ]
        }
    }
}
...
这将告诉 Claude for Desktop：
1. 有一个名为 "weather" 的 MCP 服务器
2. 通过运行 `node /绝对/路径/到/父文件夹/weather/build/index.js` 启动它
...
对于手动 MCP 服务器实现，请参考 [MCP 服务器 Java SDK 文档](/sdk/java/mcp-server)。
...
让我们实现一个 [WeatherService.java](https://github.com/spring-projects/spring-ai-examples/blob/main/model-context-protocol/weather/starter-stdio-server/src/main/java/org/springframework/ai/mcp/sample/server/WeatherService.java)，它使用 REST 客户端从美国国家气象服务 API 查询数据：
...
<Note>
Claude for Desktop 目前尚未在 Linux 上可用。
</Note>

首先，确保您已安装 Claude for Desktop。
[您可以在此处安装最新版本。](https://claude.ai/download) 如果您已经安装了 Claude for Desktop，**请确保已更新到最新版本。**

我们需要为 Claude for Desktop 配置您想使用的 MCP 服务器。
为此，请使用文本编辑器打开位于 `~/Library/Application Support/Claude/claude_desktop_config.json` 的 Claude for Desktop 应用程序配置文件。
如果文件不存在，请创建它。
...
只有至少配置了一个服务器，Claude for Desktop 中才会显示 MCP UI 元素。
    ...
    "mcpServers": {
      "spring-ai-mcp-weather": {
        ...
        "args": [
          ...
          "-jar",
          "/绝对/路径/到/父文件夹/mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar"
        ]
      }
    }
  }
    ...
    "mcpServers": {
      "spring-ai-mcp-weather": {
        ...
        "args": [
          ...
          "-jar",
          "C:\\绝对\\路径\\到\\父文件夹\\weather\\mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar"
        ]
      }
    }
  }
...
2. 通过运行 `java -jar /绝对/路径/到/父文件夹/mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar` 启动它
...
var stdioParams = ServerParameters.builder("java")
  .args("-jar", "/绝对/路径/到/父文件夹/mcp-weather-stdio-server-0.0.1-SNAPSHOT.jar")
  .build();
...
<Tab title='Kotlin'>
让我们开始构建我们的天气服务器吧！[您可以在此处找到我们将要构建的完整代码。](https://github.com/modelcontextprotocol/kotlin-sdk/tree/main/samples/weather-stdio-server)
...
首先，如果您尚未安装 `java` 和 `gradle`，让我们安装它们。
...
```kotlin build.gradle.kts
plugins {
    kotlin("plugin.serialization") version "您的 Kotlin 版本"
...
```groovy build.gradle
plugins {
    id 'org.jetbrains.kotlin.plugin.serialization' version '您的 Kotlin 版本'
    id 'com.github.johnrengelman.shadow' version '8.1.1'
}
```
</CodeGroup>
...
接下来，让我们添加用于查询和转换美国国家气象服务 API 响应的函数和数据类：

```kotlin
// 扩展函数，根据给定的纬度和经度获取预报信息
...
server.addTool(
    name = "get_alerts",
    description = """
        获取美国某州的天气警报。输入为两位美国州代码（例如 CA、NY）
    """.trimIndent(),
    ...
    if (state == null) {
        return@addTool CallToolResult(
            content = listOf(TextContent("'state' 参数是必需的。"))
        )
    }
...
server.addTool(
    ...
    if (latitude == null || longitude == null) {
        return@addTool CallToolResult(
            content = listOf(TextContent("'latitude' 和 'longitude' 参数是必需的。"))
        )
    }
...
确保运行 `./gradlew build` 来构建您的服务器。这是让服务器连接的重要一步。
...
```json MacOS/Linux
{
    "mcpServers": {
        "weather": {
            ...
            "args": [
                "-jar",
                "/绝对/路径/到/父文件夹/weather/build/libs/weather-0.1.0-all.jar"
            ]
        }
    }
}
...
```json Windows
{
    "mcpServers": {
        "weather": {
            ...
            "args": [
                "-jar",
                "C:\\路径\\到\\父文件夹\\weather\\build\\libs\\weather-0.1.0-all.jar"
            ]
        }
    }
}
...
这将告诉 Claude for Desktop：
1. 有一个名为 "weather" 的 MCP 服务器
2. 通过运行 `java -jar /绝对/路径/到/父文件夹/weather/build/libs/weather-0.1.0-all.jar` 启动它
...
<Tab title='C#'>
让我们开始构建我们的天气服务器吧！[您可以在此处找到我们将要构建的完整代码。](https://github.com/modelcontextprotocol/csharp-sdk/tree/main/samples/QuickstartWeatherServer)
...
### 设置您的环境
首先，如果您尚未安装 `dotnet`，让我们安装它。您可以从 [官方网站 Microsoft .NET](https://dotnet.microsoft.com/download/) 下载 `dotnet`。验证您的 `dotnet` 安装：
```bash
dotnet --version
```
现在，让我们创建并设置您的项目：
...
```
<Note>
创建 `ApplicationHostBuilder` 时，请确保使用 `CreateEmptyApplicationBuilder` 而不是 `CreateDefaultBuilder`。这可以确保服务器不会向控制台写入任何额外消息。这仅对使用 STDIO 传输的服务器是必要的。
</Note>

此代码设置了一个基本的控制台应用程序，使用 Model Context Protocol SDK 创建了一个带有标准输入/输出传输的 MCP 服务器。

### 天气 API 辅助函数
接下来，定义一个带有工具执行处理程序的类，用于查询和转换美国国家气象服务 API 的响应：
        ...
        [Description("要获取警报的美国州。")] string state)
    {
        ...
        return string.Join("\n--\n", alerts.Select(alert =>
        {
            ...
            return $"""
                    ...
                    地区: {properties.GetProperty("areaDesc").GetString()}
                    严重性: {properties.GetProperty("severity").GetString()}
                    描述: {properties.GetProperty("description").GetString()}
                    指令: {properties.GetProperty("instruction").GetString()}
                    """;
        }));
    }

    [McpServerTool, Description("获取某地点的天气预报。")]
    public static async Task<string> GetForecast(
        HttpClient client,
        [Description("地点纬度。")] double latitude,
        [Description("地点经度。")] double longitude)
    {
...
这将告诉 Claude for Desktop：
1. 有一个名为 "weather" 的 MCP 服务器
2. 通过运行 `dotnet run /绝对/路径/到/项目` 启动它
保存文件，然后重启 **Claude for Desktop**。
...
如果您的服务器未被 Claude for Desktop 识别，请转到[故障排除](#troubleshooting)部分查看调试建议。
...
Claude.app 相关的 MCP 日志会写入到 `~/Library/Logs/Claude` 中的日志文件：

- `mcp.log` 将包含有关 MCP 连接和连接失败的一般日志。
- 以 `mcp-server-SERVERNAME.log` 命名的文件将包含来自指定服务器的错误（stderr）日志。

您可以运行以下命令列出最近的日志并跟踪任何新的日志：
```bash
# 检查 Claude 的日志是否有错误
...
这不是错误 - 它只是意味着该州当前没有天气警报。尝试其他州或在恶劣天气期间检查。
</Accordion>

</AccordionGroup>

<Note>
有关更高级的故障排除，请查看我们的[调试 MCP 指南](/docs/tools/debugging)
</Note>
