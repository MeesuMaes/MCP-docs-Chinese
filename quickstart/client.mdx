---
title: "为客户端开发者"
description: "开始构建您自己的客户端，使其能够与所有 MCP 服务器集成。"
---

在本教程中，您将学习如何构建一个连接到 MCP 服务器的基于 LLM 的聊天机器人客户端。建议您先完成 [服务器快速入门](/quickstart/server)，它将指导您完成构建第一个服务器的基础知识。
...
```bash
echo "ANTHROPIC_API_KEY=<您的密钥在此处>" > .env
```
...
首先，让我们在 `index.ts` 中设置导入并创建基本的客户端类：
...
- 如果相对路径不起作用，请使用绝对路径
- 对于 Windows 用户，请确保在路径中使用正斜杠 (/) 或转义反斜杠 (\\)
- 验证服务器文件具有正确的扩展名（Node.js 为 .js，Python 为 .py）
...
此示例展示如何构建一个交互式聊天机器人，该机器人结合了 Spring AI 的模型上下文协议 (MCP) 和 [Brave Search MCP 服务器](https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search)。该应用程序创建了一个由 Anthropic 的 Claude AI 模型驱动的对话界面，可以通过 Brave Search 执行互联网搜索，从而实现与实时网络数据的自然语言交互。
...
1. 安装 npx (Node Package eXecute)：
   首先，确保安装 [npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm)，然后运行：
   ```bash
   npm install -g npx
   ```
...
3. 设置您的 API 密钥：
   ```bash
   export ANTHROPIC_API_KEY='您的 Anthropic API 密钥在此处'
   export BRAVE_API_KEY='您的 Brave API 密钥在此处'
   ```
...
<Warning>
请确保您的 `ANTHROPIC_API_KEY` 和 `BRAVE_API_KEY` 密钥保持安全！
</Warning>
...
使用 Spring AI 的 ChatClient 和 MCP 工具集成实现聊天机器人：

```java
var chatClient = chatClientBuilder
    .defaultSystem("您是一个有用的助手，擅长 AI 和 Java。")
...
应用程序将启动一个交互式聊天会话，您可以在其中提出问题。聊天机器人将在需要时使用 Brave Search 从互联网查找信息以回答您的查询。
...
对于基于 WebFlux 的应用程序，您可以使用 WebFlux 启动器代替：
...
suspend fun connectToServer(serverScriptPath: String) {
    try {
        ...
        println("已连接到服务器，工具包括：${tools.joinToString(", ") { it.tool().get().name() }}")
    } catch (e: Exception) {
...
另外创建一个辅助函数，将 `JsonObject` 转换为 Anthropic 的 `JsonValue`：
```kotlin
...
<Note>
如果您正在继续服务器快速入门中的天气教程，您的命令可能类似于：`java -jar build/libs/kotlin-mcp-client-0.1.0-all.jar .../samples/weather-stdio-server/build/libs/weather-stdio-server-0.1.0-all.jar`
</Note>
...
2. **安全性**
   - 将 API 密钥和机密安全存储在 `local.properties`、环境变量或密钥管理器中
...
dotnet user-secrets set "ANTHROPIC_API_KEY" "<您的密钥在此处>"
```
...
这创建了一个可以从用户机密中读取 API 密钥的 .NET 控制台应用程序的开端。
...
var (command, arguments) = args switch
{
    ...
    _ => throw new NotSupportedException("提供了不受支持的服务器脚本。支持的脚本包括 .py、.js 或 .csproj")
};
...
{
    Console.WriteLine($"已连接到服务器，工具包括：{tool.Name}");
}
```
<Note>
请确保为命名空间添加 `using` 语句：
...
这配置了一个 MCP 客户端，将连接到作为命令行参数提供的服务器。然后，它列出连接服务器的可用工具。
...
### 3. 查询处理
* 利用 [Microsoft.Extensions.AI](https://learn.microsoft.com/dotnet/ai/ai-extensions) 实现聊天客户端。
...
```
<Note>
如果您正在继续服务器快速入门中的天气教程，您的命令可能类似于：`dotnet run -- path/to/QuickstartWeatherServer`。
</Note>
